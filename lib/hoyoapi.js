...
function parseStarChallenge(raw, server) {
  ...
  return { seasonName, gameVersion, end };
}
...
// footer text는 <t:...> 타임스탬프 태그를 렌더링하지 않으므로(디스코드 제약),
// 실시간 갱신되는 타임스탬프는 별도 필드로 넣습니다.
function timestampField() {
  const epochSeconds = Math.floor(Date.now() / 1000);
  return { name: "", value: `⟳ ${footerTextKST()} • <t:${epochSeconds}:R>`, inline: false };
}
...
const GAME_VERSION_FALLBACK = "정보 없음";
